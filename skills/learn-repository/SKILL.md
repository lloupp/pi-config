...
O diretório `memory/` e `install-pi-config.sh` são ignorados automaticamente em scripts de backup.

Para migrar sua configuração do Pi:

```bash
# No ambiente atual
mkdir -p ~/pi-config
cp -r ~/.pi/agent/AGENTS.md ~/.pi/agent/settings.json ~/.pi/agent/prompts ~/.pi/agent/skills ~/.pi/agent/extensions ~/.pi/agent/themes ~/pi-config/

# No novo ambiente
bash ~/pi-config/install-pi-config.sh   # instala globalmente
```
